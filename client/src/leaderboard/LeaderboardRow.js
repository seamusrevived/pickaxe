...
export function LeaderboardRow(props) {
    ...
    return [
        ...
        <div className={`${baseCssClasses} leaderboard__cell--weeks leaderboard__cell--numerical`}
             data-testid={`leaderboard-row-weeks-${props.name}`}
             key={`leaderboard-row-weeks-${props.name}`}>
            {props.weeks}
        </div>,
        <div className={`${baseCssClasses} leaderboard__cell--picks leaderboard__cell--numerical`}
